...
public interface GoodsMapper extends BaseMapper<Goods> {
    ...
    @Update(" UPDATE Goods_seckil_center SET num = num - 1  WHERE id = #{id}  AND num>0")
...
@Update("update Goods_seckill_center set num = num - 1, version= version + 1 where goods_id = #{goodsId} and num > 0 and version = #{version}")
    ...
    @Select("select num from Goods_seckill_center  where goods_id= #{goodsId}")
    int ListGoodsSeckillNum(@Param("goodsId")long goodsId);
