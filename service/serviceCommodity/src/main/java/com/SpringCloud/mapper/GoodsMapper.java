...
@Update("update Goods_seckill_center set num = num - 1, version= version + 1 where goods_id = #{goods_id} and num > 0 and version = #{version}")
    ...
    @Select("select * from Goods_seckill_center  where goods_id = #{goodsId}")
     public Goods selectByGoodsId(@Param("goodsId") long goodsId);
}
